...
%token<node> T_IMPORT T_CLASS T_PACKAGE T_MAIN T_ARGS T_SOP
%token<node> T_TYPEDEF T_STATIC T_NEW
%token<node> T_PUBLIC T_PRIVATE T_PROTECTED
%token<node> T_OC T_CC T_OF T_CF T_OS T_CS
%token<node> T_CHAR T_INT T_FLOAT T_VOID T_STRING 
%token<node> T_IF T_ELSE T_FOR T_CONTINUE T_BREAK T_RETURN
%token<node> T_EQ T_ADD_ASSIGN T_SUB_ASSIGN T_MUL_ASSIGN T_DIV_ASSIGN 
%token<node> T_ADD T_SUB T_MUL T_DIV T_MOD 
%token<node> T_INC_OP T_DEC_OP T_LE T_GE  T_DEQ T_LT T_GT
%token<node> T_AND_OP T_OR_OP T_NE_OP T_BIT_AND T_BIT_OR T_NOT
%token<node> T_COMMA T_COL T_DOT T_SEMC T_EXP T_QUE
