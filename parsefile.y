...
extern int yyscope;
int xpos =-1;
int idx =-1; // A global variable to hold the number of entries in the symbol tabT_LE
int addr=0;
//int* addr;
struct SymTable
{
...
array_memalloc : T_IDENTIFIER T_EQ T_NEW T_INT T_OS T_NUM T_CS T_SEMC {xpos = mapping($1,yyscope); 
				...
				else {yyerror("array already declared");} 
				;
...
//int
dec_int: T_IDENTIFIER T_EQ int_e    	{if(mapping($1,yyscope)==-1){idx++; strcpy(st[idx].idName,$1); st[idx].type =0; 						st[idx].value = $3; st[idx].scope = yyscope;  st[idx].line_no = yylineno; } 
...
//float
dec_float: 	T_IDENTIFIER T_EQ float_e		{if(mapping($1,yyscope)==-1) {idx++; strcpy(st[idx].idName,$1); st[idx].type =1;
				st[idx].f_val = $3; 			st[idx].scope = yyscope;  st[idx].line_no = yylineno;}  
				else {yyerror("variable already declared");} }
   		
   		| dec_float T_COMMA T_IDENTIFIER    	{if(mapping($3,yyscope)==-1) { idx++; strcpy(st[idx].idName,$3);  
   			st[idx].type =1; st[idx].scope = yyscope;  st[idx].line_no = yylineno;} else {yyerror(errMess);}}
...
//char 					
dec_char: T_IDENTIFIER T_EQ char_e		{if(mapping($1,yyscope)==-1) {idx++; strcpy(st[idx].idName,$1); st[idx].type =2; 					st[idx].c_val = $3; st[idx].scope = yyscope;  st[idx].line_no = yylineno;}   
				else {yyerror("variable already declared");} }
...
dec_string: T_IDENTIFIER T_EQ str_e			{if(mapping($1,yyscope)==-1) {idx++; strcpy(st[idx].idName,$1); st[idx].type =3; 
			strcpy(st[idx].s_val,$3); st[idx].scope = yyscope;  st[idx].size=strlen(st[idx].s_val); st[idx].line_no = yylineno;} 				else {yyerror("variable already declared");} }
...
int_e: int_e T_MUL int_e   	 {$$ = $1 * $3;}
     ...
     | T_IDENTIFIER   		{xpos = mapping($1,yyscope); if(xpos!=-1 && st[xpos].type==0) { $$ = st[xpos].value; } 	
     		else {yyerror("Trying to use undeclared variable");} }
     | T_NUM   			 {$$ =  $1; }
...
float_e: T_IDENTIFIER   	{xpos = mapping($1,yyscope); if(xpos!=-1 && st[xpos].type==1) { $$ = st[xpos].f_val; } 
	 ...
	 | T_DECIMAL   				{$$ = $1; }
	 | T_IDENTIFIER T_EQ float_e   	{xpos = mapping($1,yyscope); if(xpos!=-1 && st[xpos].type==1) { st[xpos].f_val = $3; } else 						{yyerror("Variable not declared");}}
	 ;

char_e: T_IDENTIFIER   		{xpos = mapping($1,yyscope); if(xpos!=-1 && st[xpos].type==2) { $$ = st[xpos].c_val; } 
			else {yyerror("Variable not declared");} }
	 | T_CHARACTER  				{$$ = $1; }
	 | T_IDENTIFIER T_EQ char_e  	{xpos = mapping($1,yyscope); if(xpos!=-1 && st[xpos].type==2) { st[xpos].c_val = $3; } else {yyerror("Variable not declared");}}
	 ;
					
str_e: T_IDENTIFIER  		{xpos = mapping($1,yyscope); if(xpos!=-1 && st[xpos].type==3) { $$ = strcpy(st[idx].s_val,$1); } 
			else {yyerror("Variable not declared");} }
	 | T_STRING_LITERAL				{$$ = $1; }
	 | T_IDENTIFIER T_EQ str_e  	{xpos = mapping($1,yyscope); if(xpos!=-1 && st[xpos].type==3) { strcpy(st[idx].s_val,$3); 				st[idx].size=strlen(st[idx].s_val)	;  } else {yyerror("Variable not declared");}}
	 ;

if_construct: T_IF T_OC expression T_CC T_OF stmnt T_CF  else 	
		;
